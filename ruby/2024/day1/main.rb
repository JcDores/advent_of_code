...
class Main
  attr_accessor :locations, :file_name, :file, :distance
  def initialize(file_name)
    @file = File.open(file_name, 'r')
    @location_ids = @file.read.split("\n")
    @left_list = []
    @right_list = []
    @answer = 0
  end

  def process_locations_ids
    @location_ids.each do |location_id|
      left_location, right_location = location_id.split("  ")
  ...
  def calculate_distance
    @answer = 0
    @left_list.each_with_index do |left_id, index|
      @answer += (left_id - @right_list[index]).abs
    end
  end
